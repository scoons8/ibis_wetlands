...
# Summarize the water data and join blob ID info -----
  ...
  stateHydro <- hydroTerm %>% 
        group_by(idPoly, siteName, year, term, ecoHydro) %>%                    # calculate the total water for each polygon in each year     
...
# Box Plot -----
    ...
    stateBox <- stateHydro %>%    
      group_by(term, state, year) %>%
      summarise(wetSum = sum(wetHa)) %>% 
      mutate(wetSum = 2.471*wetSum) #<-------- Converted everything to acres
    
    ggplot(stateBox, aes(x = term, y = wetSum, group = term, fill = term)) +  
      geom_boxplot() +
...
# Summarize by wetland type -----
  ...
  wetHydro <- stateHydro03 %>% 
    group_by(state, wetType, year, term) %>%           
    summarise(wetHa = sum(wetHa)) %>% 
    ungroup() %>% 
