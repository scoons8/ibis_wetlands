...
# Load data -----
  ...
  AllDat <- fread('01_data/11_RF_vars_clean/RFvars05.csv')
...
# Filter by ecoregion - Need to run for each region (x8) -----
    ...
    rmDat01 <- rmDat %>% 
      filter(region == 'Great Basin-Colorado Plateau') # <-- Change region here
...
# Wet_ha response var -----
  ...
  Wet_rmUsa <- rmDat01 %>% 
    dplyr::select(temporary, pr,aet,ro,swe,tmin,AG_ha) %>% # <-- switch out response var here 
    data.frame()
...
# ~. is shorthand for telling it to use wetHa as response? -----
  ...
  src_Wet_rmUsa <- rfsrc(temporary~ ., data=Wet_rmUsa, importance = TRUE, ntree=5000) 
