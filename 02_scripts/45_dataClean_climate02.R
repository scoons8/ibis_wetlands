...
# Read in data -----
  ...
  climVars <- fread("01_data/10_climate_vars_raw/climateVarTest.csv")
  tmin <- fread("01_data/10_climate_vars_raw/tminTest02.csv")
...
# Clean seasonal data for RF analysis ------
  ...
  hydro01 <- hydroGra %>% 
    group_by(siteName, ecoHydro, HUC4, month, year) %>% 
